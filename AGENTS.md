...
- **Two page-container tiers, chosen by what the page actually is, not by default.** `.site-container` (1440px max-width, flat 24px padding) is reserved for browsing/grid pages that benefit from the extra width — the Browse homepage and Garage (both render a car-card grid). `.detail-container` (1200px max-width, padding steps 20px→24px→40px across breakpoints) is the standard for every other public page — Contact, About, Terms, Privacy, the report-a-mistake page, and any future non-browsing page (a new legal page, a new form page, etc.) — matching the car detail page's own width/padding exactly, not a one-off. When adding a new page, ask "is this a car grid, or standalone content?" before picking a container. Internal tooling (`/admin`) is a separate, deliberate, disclosed exception — it uses its own narrower widths because a dense table/form stretched to full content width visibly hurts usability; don't "fix" that inconsistency by forcing it to match without discussing it first.
